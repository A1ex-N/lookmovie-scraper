...
class Scraper:
    ...
    def __init__(self, database_name, start_page=1, end_page=0):
    ...
    def start_scraping_test(self):
        ...
        for movie_node in self.movie_list_node.iter():
            self.movie_list.append(MovieInfo(movie_node, 0))
    ...
    async def get_episode(self, client: httpx.AsyncClient, page_num: int):
        ...
        try:
            ...
            for movie_node in self.movie_list_node.iter():
                self.movie_list.append(MovieInfo(movie_node, self.current_page))
